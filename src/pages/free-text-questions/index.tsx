...
export default function FreeTextQuestionsPage() {
  const { language, getSettingValue } = useAppSettings()
  const [questions, setQuestions] = useState<FreeTextQuestion[]>([])
  ...
  const loadQuestions = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetchFreeTextQuestions({ ...filters, lang: language })
  ...
  const handleFilterChange = (newFilters: FreeTextQuestionFilters) => {
  ...
  return (
    <div className="ftq-page">
      ...
      {!loading && !error && questions.length === 0 && (
        <div className="ftq-empty">
          📭 {t('noQuestions', language)}
        </div>
      )}

      {!loading && !error && questions.length > 0 && (
        <div className="ftq-pagination">
          <Pagination
