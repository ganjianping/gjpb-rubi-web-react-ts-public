...
export default function TrueFalseQuestionsPage() {
  ...
  const loadQuestions = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetchTrueFalseQuestions({ ...filters, lang: language })
  ...
  const handleFilterChange = (newFilters: TrueFalseQuestionFilters) => {
  ...
  return (
    <div className="tfq-page">
      ...
      {!loading && !error && questions.length === 0 && (
        <div className="tfq-empty">
          📭 {t('noQuestions', language)}
        </div>
      )}

      {!loading && !error && questions.length > 0 && (
        <div className="tfq-pagination">
          <Pagination
